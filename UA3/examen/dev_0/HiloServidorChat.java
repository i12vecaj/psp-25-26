...
public class HiloServidorChat extends Thread {
	...
	public HiloServidorChat(Socket s, ComunHilos comun) {
		...
		try {
			...
			fentrada = new DataInputStream(socket.getInputStream());// Creamos el flujo de entrada
		} catch (IOException e) {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				comun.setACTUALES(comun.getACTUALES() - 1); // Reducimos el numero de conexiones actuales ya que se ha desconectado 1
				System.out.println("NUMERO DE CONEXIONES ACTUALES: " + comun.getACTUALES());
	...
	private void EnviarMensajesaTodos(String texto) { // Funcion que envia los mensajes al chat, necesita el texto a enviar
		int i;
