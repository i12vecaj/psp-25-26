...
public class ClienteChat implements Runnable {
	...
	public void run() {
		...
		while (repetir) {
			try {
				texto =  fentrada.readUTF(); //Leemos del servidor el texto enviado por otros clientes para posteriormente mostrarlo por pantalla
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			cliente =   new ClienteChat(s, nombre); //Creamos un objeto ClienteChat, mediante hilos podemos tener varios clientes
		...
		while(repetir)
		{
			String texto =  bufferedReader.readLine(); //Leemos por consola el texto a enviar al chat
			if(texto.length()>0)
			{
				if(texto.equals("*"))
				{
