...
public class HiloServidorChat extends Thread {
	...
	public HiloServidorChat(Socket s, ComunHilos comun) {
		...
		try {
			...
			fentrada = new DataInputStream(socket.getInputStream()); // CAMBIADO: obtiene flujo de entrada
		} catch (IOException e) {
		...
		while (true) {
			...
			try {
				...
				if (cadena.trim().equals("*")) {// EL CLIENTE SE DESCONECTA
					...
					System.out.println("NUMERO DE CONEXIONES ACTUALES: " + comun.getACTUALES());
					break; // CAMBIADO: sale del bucle cuando recibe asterisco
				}
				...
				comun.setACTUALES(comun.getACTUALES() - 1); // CAMBIADO: cambia cuando un cliente se desconecta
				System.out.println("NUMERO DE CONEXIONES ACTUALES: " + comun.getACTUALES());
	...
	private void EnviarMensajesaTodos(String texto) { // CAMBIADO: envia mensaje a todos
		int i;
