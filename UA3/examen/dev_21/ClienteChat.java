...
public class ClienteChat implements Runnable {
	...
	public void run() {
		...
		while (repetir) {
			try {
				texto = fentrada.readUTF(); // CAMBIADO: lee mensaje del servidor
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			cliente = new ClienteChat(s, nombre); // CAMBIADO: crea instancia del cliente
		...
		while(repetir)
		{
			String texto = bufferedReader.readLine(); // CAMBIADO: lee del teclado
			if(texto.length()>0)
			{
				if(texto.equals("*"))
				{
					repetir = false;
					cliente.repetir = false; // CAMBIADO: detiene el hilo run() del cliente
