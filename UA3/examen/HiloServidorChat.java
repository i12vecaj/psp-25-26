...
public class HiloServidorChat extends Thread {
    ...
    public HiloServidorChat(Socket s, ComunHilos comun) {
        ...
        try {
            ...
            fentrada = new DataInputStream(socket.getInputStream()); // Respuesta por parte del servidor
        } catch (IOException e) {
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                EnviarMensajesaTodos(comun.getUltimo()); // Recibe el ultimo mensaje del chat

            } catch (Exception e) {// EL CLIENTE SE DESCONECTA A LO BRUTO Ctrl+C o similar
                comun.setACTUALES(comun.getACTUALES()); // Recibe el ultimo mensaje
                System.out.println("NUMERO DE CONEXIONES ACTUALES: " + comun.getACTUALES());
    ...
    private void EnviarMensajesaTodos(String texto) {  // Texto que se envia a todos cuando s
        int i;
