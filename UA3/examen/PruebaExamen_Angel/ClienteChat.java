...
public class ClienteChat implements Runnable {
	...
	public void run() {
		...
		while (repetir) {
			try {
				texto = fentrada.readUTF();//es el canal donde escuchamos y alponer read, al recibir un mesanje se reescribe y lo muestra en pantalla
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			cliente = new ClienteChat(s, nombre);//aqui le pasamos el socket s y el nombre de usuario para iniciarlo
		...
		while(repetir)
		{
            String texto = bufferedReader.readLine();//aqui lo que hemos hecho es creaer un buffer reader para escuchatodo lo que el cliente escriba en el chat, como bien pone en los apuente es mas eficiente para leer linea a linea
			if(texto.length()>0)
			{
				if(texto.equals("*"))
				{
