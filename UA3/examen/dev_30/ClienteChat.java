...
public class ClienteChat implements Runnable {
	...
	public void run() {
		...
		while (repetir) {
			try {
				texto = fentrada.readUTF();// falta "fentrada.readUTF()" de este modo el cliente puede ver los mensajes existentes de los demas clientes
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			cliente = new ClienteChat(s, nombre);// faltaba crear el cliente chat pasandole como prametros el socket y el nombre para poder iniciar el hilo conectarlo con el servidor
		...
		while(repetir)
		{
			String texto = bufferedReader.readLine();// faltaba "bufferedReader.readLine()" para poder ingresar el menjase que el cliente queisiera escribir para los demas clientes
			if(texto.length()>0)
			{
				if(texto.equals("*"))
				{
