...
public class ClienteChat implements Runnable {
    ...
    public void run() {
        ...
        while (repetir) {
            try {
                texto = fentrada.readUTF(); // Lee nuestro objeto nombre
    ...
    public static void main(String args[]) throws IOException {
        ...
        try {
            ...
            cliente = new ClienteChat(s, nombre); // Creamos nuestro cliente con el socker y nuestro nombre
        ...
        while(repetir)
        {
            String texto = bufferedReader.readLine(); //Lee nuestro texto que introduciimos
            if(texto.length()>0)
            {
                if(texto.equals("*"))
                {
