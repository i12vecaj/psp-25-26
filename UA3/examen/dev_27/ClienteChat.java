...
public class ClienteChat implements Runnable {
	...
	public void run() {
		...
		while (repetir) {
			try {
				texto = this.nombre; // AQUI ME DA ERROR AAAAA HE PROBADO DE TODO Y CADA CAMBIO ES UN ERROR DISTINTO
	...
	public static void main(String args[]) throws IOException {
		int puerto = 44444; // aquí está el puerto que busca el cliente. Rellenar en ServidorChat
		Socket s = null;
		...
		while (repetir) {
			String texto = bufferedReader.readLine(); // AQUI LEE TEXTO DE EXTRADA
			if (texto.length() > 0) {
				if (texto.equals("*")) {
					repetir = false;
