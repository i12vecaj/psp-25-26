...
public class HiloServidorChat extends Thread {
	...
	public HiloServidorChat(Socket s, ComunHilos comun) {
		...
		try {
			...
			fentrada = new DataInputStream(socket.getInputStream()); // flujo de entrada como inptStream
		} catch (IOException e) {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				comun.setACTUALES(comun.getACTUALES() - 1); //numero de conexiones actuales -1
				System.out.println("NUMERO DE CONEXIONES ACTUALES: " + comun.getACTUALES());
	...
	private void EnviarMensajesaTodos(String texto) { // envia el texto, que es un string, a todos los clientes
		int i;
