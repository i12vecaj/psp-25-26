...
public class HiloServidorChat extends Thread {
	...
	public HiloServidorChat(Socket s, ComunHilos comun) {
		...
		try {
			...
			fentrada = new DataInputStream(s.getInputStream()); // Usamos este comando coger el mensaje enviado por el cliente
		} catch (IOException e) {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				comun.setACTUALES(comun.getACTUALES() - 1);// Restamos uno a la cantidad de clientes conectados
															// ya que uno se desconecto a la fuerza
				System.out.println("NUMERO DE CONEXIONES ACTUALES: " + comun.getACTUALES());
	...
	private void EnviarMensajesaTodos(String texto) { // Ponemos que el metodo reciba el texto que debe enviar a todos los clientes
		int i;
