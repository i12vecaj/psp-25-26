...
public class ClienteChat implements Runnable {
	...
	public void run() {
		String texto = "";
		while (repetir) {
			try { // Aqui el servidor muestra al cliente los mensajes que escriben el resto de
					// clientes en el chat, usando la variable que recoge los mensajes.
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			cliente = new ClienteChat(s, nombre); // Aqui instancio al Cliente, lo que se traduce a que le asigno el
													// socket y el nombre
			new Thread(cliente).start(); // al cliente cuando entra
		...
		while (repetir) {
			String texto = bufferedReader.readLine(); // Aqui creo un buffered reader, que basicamente lo que hace es
														// leer lo que el usuario pone por teclado para mas tarde poder
														// enviarlo al chat
			if (texto.length() > 0) {
				if (texto.equals("*")) {
					repetir = false;
