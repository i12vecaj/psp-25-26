...
public class ClienteChat implements Runnable {
	...
	public void run() {
		...
		while (repetir) {
			try {
				texto = fentrada.readUTF(); // leemos UTF puesto que la salida se hace con writeUTF
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			cliente = new ClienteChat(s, nombre); // "Construimos" el hilo del cliente
		...
		while (repetir) {
			String texto = bufferedReader.readLine(); // leemos el texto
			if (texto.length() > 0) {
				if (texto.equals("*")) {
					repetir = false;
