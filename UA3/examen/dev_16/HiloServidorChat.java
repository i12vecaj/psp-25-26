...
public class HiloServidorChat extends Thread {
	...
	public HiloServidorChat(Socket s, ComunHilos comun) {
		...
		try {
			...
			fentrada = new DataInputStream(socket.getInputStream()); // Flujo de entrada para leer los mensajes
		} catch (IOException e) {
		...
		while (true) {
			...
			try {
				...
				if (cadena.trim().equals("*")) {// EL CLIENTE SE DESCONECTA
					comun.setACTUALES(comun.getACTUALES() - 1); // Resta 1 a la cantidad de clientes actuales conectados
					System.out.println("NUMERO DE CONEXIONES ACTUALES: " + comun.getACTUALES());
				}
	...
	private void EnviarMensajesaTodos(String texto) { // el parametro necesario es un String
		int i;
